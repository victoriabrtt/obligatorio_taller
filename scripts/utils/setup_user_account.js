...
async function main() {
  ...
  if (!userAddress) {
    ...
    console.error("Example: USER_ADDRESS=0x123... npx hardhat run scripts/utils/setup_user_account.js --network localhost");
    process.exit(1);
  }
  ...
  try {
    ...
    try {
      ...
      console.log("Tokens staked for voting");
    } catch (error) {
      console.log("Could not stake for voting, may already be staked:", error.message);
    }
    ...
    try {
      ...
      console.log("Could not stake for proposals, may already be staked:", error.message);
    }
    ...
    console.log("Setup complete! The user can now vote and create proposals after the staking time requirement is met.");
